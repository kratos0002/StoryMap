...
def extract_stories_from_ts(file_path):
    ...
    # Extract individual story objects using regex
    story_pattern = r'\{\s*id:\s*"([^"]+)".*?culturalContext:\s*"([^"]*)"(?:,\s*imageUrl:\s*"([^"]*)")?\s*\}'
    story_objects = re.findall(r'\{\s*id:[^}]+\}', stories_str, re.DOTALL)
    ...
    for story_obj in story_objects:
        ...
        author_match = re.search(r'author:\s*"([^"]+)"', story_obj)
        country_match = re.search(r'country:\s*"([^"]+)"', story_obj)
        country_code_match = re.search(r'countryCode:\s*"([^"]+)"', story_obj)
        region_match = re.search(r'region:\s*"([^"]+)"', story_obj)
        
        # Extract coordinates
        lat_match = re.search(r'lat:\s*([\d.-]+)', story_obj)
        lng_match = re.search(r'lng:\s*([\d.-]+)', story_obj)
        
        reading_time_match = re.search(r'readingTimeMinutes:\s*(\d+)', story_obj)
        
        # Extract themes array
        themes_match = re.search(r'themes:\s*\[(.*?)\]', story_obj, re.DOTALL)
        themes = []
        if themes_match:
            themes_str = themes_match.group(1)
            themes = re.findall(r'"([^"]+)"', themes_str)
        
        mood_match = re.search(r'mood:\s*"([^"]+)"', story_obj)
        preview_text_match = re.search(r'previewText:\s*"([^"]+)"', story_obj)
        
        # Extract full text - this might be multiline and complex
        full_text_match = re.search(r'fullText:\s*"([^"]+)"', story_obj)
        if not full_text_match:
            # Try to match multiline text
            full_text_match = re.search(r'fullText:\s*"(.*?)",\s*culturalContext', story_obj, re.DOTALL)
        
        cultural_context_match = re.search(r'culturalContext:\s*"([^"]*)"', story_obj)
        image_url_match = re.search(r'imageUrl:\s*"([^"]*)"', story_obj)
        ...
        story = {
            ...
            'fullText': full_text_match.group(1) if full_text_match else '',
            'culturalContext': cultural_context_match.group(1) if cultural_context_match else '',
            'imageUrl': image_url_match.group(1) if image_url_match else None
        }
...
def generate_cultural_contexts_sql(stories):
    ...
    if not values:
        return ""  # Return empty string if no cultural contexts
