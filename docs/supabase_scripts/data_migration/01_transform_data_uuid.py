...
Fixed version: Uses UUIDs for all primary keys including story IDs.
"""
...
# Dictionary to store mapping between original story IDs and new UUIDs
story_id_mapping = {}
...
def extract_stories_from_ts(file_path):
    ...
    for story_str in story_objects:
        ...
        # Get original ID and generate a UUID for it
        original_id = id_match.group(1) if id_match else ''
        new_uuid = str(uuid.uuid4())
        
        # Store the mapping between original ID and new UUID
        story_id_mapping[original_id] = new_uuid
        
        # Create story object
        story = {
            'original_id': original_id,  # Keep original ID for reference
            'id': new_uuid,  # Use UUID for database
    ...
    for i, story in enumerate(stories):
        ...
        print(f"  Original ID: {story['original_id']}")
        print(f"  New UUID: {story['id']}")
...
def generate_stories_sql(stories, embedded_texts):
    ...
    for story in stories:
        ...
        if story['original_id'] in embedded_texts:
            full_text = embedded_texts[story['original_id']]
...
# Function to generate a mapping file for original IDs to UUIDs
def generate_id_mapping_file(stories):
    mapping_content = "# Story ID Mapping\n"
    mapping_content += "# Original ID -> UUID\n\n"
    
    for story in stories:
        mapping_content += f"{story['original_id']} -> {story['id']}\n"
    
    # Write mapping to file
    mapping_path = os.path.join(OUTPUT_DIR, "story_id_mapping.txt")
    with open(mapping_path, 'w', encoding='utf-8') as f:
        f.write(mapping_content)
    
    print(f"Generated ID mapping file at {mapping_path}")
...
def main():
    ...
    generate_id_mapping_file(stories)
    ...
    full_sql += "-- Uses UUIDs for all primary keys including story IDs\n\n"
    ...
    # Write SQL to file
    output_path = os.path.join(OUTPUT_DIR, "storymap_data_migration_uuid.sql")
